...
for i=1:c %for each color channel
    ...
    fprintf('SK:Single Channel Start Loop\n');
    toc;
    ...
    fprintf('SK:Single Channel Loop End\n');
    toc;
    ...
    fprintf('SK:Solve Linear End, X length = %d\n', length(X));
    toc;
    print = 0;
    for counter=1:length(X)
        [index_x,index_y]=find(map==counter);
        im_out(index_x,index_y,i)=X(counter);
        if(round(counter * 100 / length(X)) > print)
            fprintf('SK:Counter Loop %d%% percent done.\n', print);
            print =  print + 5;
            toc;
        end
    end
